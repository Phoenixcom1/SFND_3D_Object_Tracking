...
void cropLidarPoints(std::vector<LidarPoint> &lidarPoints, float minX, float maxX, float maxY, float minZ, float maxZ, float minR)
{
    ...
    for (auto &lidarPoint : lidarPoints) {
        ...
        if (lidarPoint.x >= minX && lidarPoint.x <= maxX && lidarPoint.z >= minZ && lidarPoint.z <= maxZ &&
            lidarPoint.z <= 0.0 && abs(lidarPoint.y) <= maxY &&
            lidarPoint.r >= minR)  // Check if Lidar point is outside of boundaries
        {
...
void showLidarTopview(std::vector<LidarPoint> &lidarPoints, const cv::Size &worldSize, const cv::Size &imageSize,
                      bool bWait) {
    ...
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it) {
        float xw = static_cast<float>((*it).x); // world position in m with x facing forward from sensor
        float yw = static_cast<float>((*it).y); // world position in m with y facing left from sensor
...
void showLidarImgOverlay(cv::Mat &img, std::vector<LidarPoint> &lidarPoints, cv::Mat &P_rect_xx, cv::Mat &R_rect_xx, cv::Mat &RT, cv::Mat *extVisImg)
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        ...
        float val = static_cast<float>(it->x);
