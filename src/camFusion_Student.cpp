...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, const cv::Size &worldSize, const cv::Size &imageSize,
                   bool bWait) {
    ...
    for (auto it1 = boundingBoxes.begin(); it1 != boundingBoxes.end(); ++it1) {
        ...
        int top = static_cast<int>(1e8), left = static_cast<int>(1e8), bottom = static_cast<int>(0.0), right = static_cast<int>(0.0);
        float xwmin = 1e8, ywmin = 1e8, ywmax = static_cast<float>(-1e8);
        for (auto it2 = it1->lidarPoints.begin(); it2 != it1->lidarPoints.end(); ++it2) {
            // world coordinates
            float xw = static_cast<float>((*it2).x); // world position in m with x facing forward from sensor
            float yw = static_cast<float>((*it2).y); // world position in m with y facing left from sensor
                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC, cv::Mat *visImg)
{
    ...
    for (auto it1 = kptMatches.begin(); it1 != kptMatches.end() - 1; ++it1) { // outer kpt. loop
        ...
        cv::KeyPoint kpOuterCurr = kptsCurr.at(static_cast<unsigned long>(it1->trainIdx));
        cv::KeyPoint kpOuterPrev = kptsPrev.at(static_cast<unsigned long>(it1->queryIdx));
        ...
        for (auto it2 = kptMatches.begin() + 1; it2 != kptMatches.end(); ++it2) { // inner kpt.-loop
            ...
            cv::KeyPoint kpInnerCurr = kptsCurr.at(static_cast<unsigned long>(it2->trainIdx));
            cv::KeyPoint kpInnerPrev = kptsPrev.at(static_cast<unsigned long>(it2->queryIdx));
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    ...
    if ((prevDataVector.empty()) || (currDataVector.empty())) {
        TTC = NAN;
        return;
    }
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame) {
    ...
    int bbCount_prev = static_cast<int>(prevFrame.boundingBoxes.size());
    int bbCount_curr = static_cast<int>(currFrame.boundingBoxes.size());
