                   ...
                   const std::string &modelWeights, bool bVis)
{
    ...
    if(bVis) {
        ...
        for (auto &bBoxe : bBoxes) {
            ...
            rectangle(visImg, cv::Point(left, static_cast<int>(top - round(1.5 * labelSize.height))), cv::Point(
                    static_cast<int>(left + round(1.5 * labelSize.width)), top + baseLine), cv::Scalar(255, 255, 255),
                      cv::FILLED);
