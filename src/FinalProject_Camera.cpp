...
//    vector<string> descriptors = {"BRISK", "BRIEF", "ORB", "FREAK", "AKAZE", "SIFT"};
    ...
    myfile << "Detector Type, Descriptor Type, Frame Number, Camera TTC, Lidar TTC, (abs) TTC Delta\n";
    ...
    for(const string& det : detectors)
    {
        for (const string &des : descriptors)
        {
            ...
            string detectorType = det; //SHITOMASI, HARRIS, FAST, BRISK, ORB, AKAZE, SIFT
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth) {
                ...
                if (frameBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    matchBoundingBoxes(matches, bbBestMatches, *frameBuffer.getSecondLatest(), *frameBuffer.getLatest()); // associate bounding boxes between current and previous frame using keypoint matches
                    ...
                    for (auto it1 = frameBuffer.getLatest()->bbMatches.begin();
                        ...
                        for (auto it2 = frameBuffer.getLatest()->boundingBoxes.begin();
                             it2 != (frameBuffer.getLatest()->boundingBoxes.end()); ++it2) {
                        ...
                        for (auto it2 = frameBuffer.getSecondLatest()->boundingBoxes.begin();
                             it2 != frameBuffer.getSecondLatest()->boundingBoxes.end(); ++it2) {
            ...
            double SumAbsTTCdelata;
            for (int i = 0; i < cameraTTCs.size(); i++) {
                myfile << detectorType << "," << descriptorType << "," << i << "-" << i + 1 << "," << cameraTTCs[i]
                       << "," << lidarTTCs[i] << "," << std::abs((cameraTTCs[i] - lidarTTCs[i])) << ",\n";
                SumAbsTTCdelata += std::abs((cameraTTCs[i] - lidarTTCs[i]));
            }
            myfile << "" << "," << "" << "," << "" << "," << "" << "," << "Average TTC delta" << ","
                   << SumAbsTTCdelata / cameraTTCs.size() << ",\n";
